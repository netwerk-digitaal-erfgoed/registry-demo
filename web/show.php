...
function getMetadata() {
  var sparqlLastDateRead = "PREFIX schema: <http://schema.org/> SELECT ?postedURL ?postedDate ?lastDateRead ?ratingValue ?ratingExplanation ?validUntil WHERE {  BIND(<"+datasetUri+"> AS ?postedURL) OPTIONAL { ?postedURL schema:subjectOf/schema:validUntil ?validUntil . } OPTIONAL { ?postedURL schema:datePosted ?postedDate . } OPTIONAL { ?postedURL schema:dateRead ?lastDateRead . } OPTIONAL { ?postedURL schema:contentRating/schema:ratingValue ?ratingValue ; schema:contentRating/schema:ratingExplanation ?ratingExplanation . } } ORDER BY DESC(?lastDateRead) LIMIT 1";
