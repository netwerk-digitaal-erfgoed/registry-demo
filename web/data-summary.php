...
# deze lijst moet ook wel te automatiseren zijn...
...
if (!isset($_GET["dataset"]) || !filter_var($_GET["dataset"], FILTER_VALIDATE_URL)) {
	exit;
}

#$dataset_uri="https://www.goudatijdmachine.nl/omeka/api/items/13000";
#$dataset_uri="http://data.beeldengeluid.nl/id/dataset/0029";

$dataset_uri=$_GET["dataset"];
...
if (isset($stats["termUse"]) && count($stats["termUse"])>0) {
	$str.="<li>De resource in deze dataset maken gebruik van termen uit de volgende bronnen: ";
	$tu=array();
...
function doSPARQLcall($sparqlQueryString) {
	...
	# error_log($sparqlQueryString." >> ".$response);
