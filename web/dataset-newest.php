...
<main>
  <section class="text m-t-space m-b-space m-theme--blue">
    <div class="o-container o-container__small m-t-space">
	  ...
	  <p>Onderstaande lijst toont de <?= $shownewest ?> meest recent aangemelde datasetbeschrijvingen via de <a href="viaurl.php">Meld aan</a> pagina of de <a href="https://datasetregister.netwerkdigitaalerfgoed.nl/api/static/index.html">Dataset Register API</a>.</p>
...
SELECT DISTINCT ?dataset ?title ?publisherName ?postedDate WHERE {
  ...
  FILTER(?postedURL!=<https://demo.netwerkdigitaalerfgoed.nl/datasets/kb/2.html>)  
...
} ORDER BY DESC(?postedDate) LIMIT 20
`;
...
function showDatasets(sparqlresult) {
  ...
  for (var prop in sparqlresult.results.bindings) {
	...
	postedDate = sparqlresult.results.bindings[prop].postedDate.value;
	...
	link.setAttribute("title", "Aangemeld op "+postedDate.substr(8,2)+"-"+postedDate.substr(5,2)+"-"+postedDate.substr(0,4)+" ("+postedDate.substr(11,5)+")");
